	...
	if(!empty($getWaiting))
	{
		foreach ($getWaiting as $value)
		{
			if($value['user_1']!=0 && $value['status_1']==0)
			{
				...
				$getProof = $db->numRow("select * from `payer_proof` where `waiting_id`=? AND `payer_user_id`=? AND `payee_user_id`=?",array($value['id'],$value['user_1'],$value['user_id']));
				if($getProof > 0){
					// alredy uploaded
				}else{
					// call finction
			...
			{
				// check proof will uploded or not
				$getProof = $db->numRow("select * from `payer_proof` where `waiting_id`=? AND `payer_user_id`=? AND `payee_user_id`=?",array($value['id'],$value['user_2'],$value['user_id']));
			if($getProof > 0){
				// alredy uploaded
			}else{
				// call finction
